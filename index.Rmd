...
![](https://www.youtube.com/watch?v=7i_7DfCorcg)
...
For the purposes of this demonstration, we are going to use two examples from the Humpback Whale data set.
The number of Humpback whales in the data set is 23,581, large enough to assume that the characteristics of interest  
...
The table below shows the number and percentage of whales of each sex. 
...
Let's suppose we selected a sample of n=400, the mean length is 13.39m, and the 95% Confidence interval [13.257;13.534]. The true average length value, that will usually be unknown, is 13.30m, which is included in the confidence interval. However, there are lots of other values, infinite to be precise, that are included in the confidence interval. According to our sample and the confidence interval  we obtained from it, the mean population length of the Humpback whales could be 13.27m, or 13.41m, or 13.50m or any other value within the interval limits. And there is still a small chance, 5%, that the true population mean length is outside the limits, either way. 
What if we generated the 99% confidence interval instead from the same sample? the 99% CI would be [13.213;13.578]. Now, there is just 1% probability that the true value lies beyond the CI limits, but this comes at a cost: the 99% CI is wider than the 95% CI. 
...
The computation of the 95% Confidence Interval as we have seen it comes from the fact that the mean of a sufficiently large sample size (a sample size of *n* greater than 5 could be sufficient) is *normally distributed*, according to the *Central Limit Theorem*. The previous statement also holds for proportions, although larger sample sizes may be required. 

There is a range of methods that do not require normality and can work in any circumstances. These methods are based on *resampling*: the creation of new samples based on one observed sample. One such method is *bootstrapping*.

The bootstrap creates multiple resamples (with replacement) from a single set of observations and obtains the (bootstrap) sampling distribution of the characteristic of interest in the population (e.g., mean, proportion, etc.). It is, in a way, the equivalent of taking many different samples and observing how the characteristic of interest is distributed. Hence the name of *bootstrap*, the magical idea of pulling oneself up by one's own bootstraps.

Let's go back to the whale dataset. We have a sample of 400 whales obtained by Simple Random Sampling. We can then resample n=200 units from it *with replacement*, meaning that we can have the same whale more than once in the first resample. We then compute the estimate of interest: the mean, or the proportion of males. We then repeat the process a second time and obtain a second estimate, and then a third, and so on, up to a large number of resamples, for example, 5000. These 5000 resample means (or proportions) can be plotted in a histogram, and we can also get the two values that include the central 95% of the resample estimates, leaving the 2.5% most extreme on both sides out. This is the  95% bootstrap Confidence Interval.

The figure below shows the 95% Bootstrap Confidence Interval for the population mean whale length. 

```{r, out.width="80%", echo = FALSE}
img <- readPNG("img/BootstrapCI.png")
grid.raster(img)
```

The 95% Bootstrap CI computed with the original sample of n=400 is [13.201; 13.587], while the 95% CI computed using the normal distribution is [13.257;13.534], and the original sample mean is 13.396. We could also compute the 99% bootstrap CI by leaving out the most extreme 0.5% of observations on both sides: [13.213; 13.578], or a Confidence Interval for any other confidence level.

Both 95% CIs —the classical one that uses the normal distribution and the bootstrap CI,

-   contain the true population value of 13.289, 

-   were obtained from the original sample of n=400

The advantage of using a Bootstrap Confidence Interval is that there is no need to assume that our observations, or the underlying populations, are normally distributed. It can always be used, even if our data is normally distributed.
...
![](https://www.youtube.com/watch?v=UDB5Uon7ZYg)
...
```{r q6, echo = FALSE}
question("Select all the correct statements regarding the sampling process you would use",
         answer("It will be more efficient, in terms of costs and time, to divide the basin into clusters, for example, each tributary would be one cluster and random sample clusters and then random sample within selected clusters to obtain the catchment sites", correct = TRUE, message ="It is usually more cost- and time-efficient to use a clustered design\n"),
         answer("Cluster sampling is more time and cost efficient than Simple Random Sampling", correct = TRUE, message = "Stratified sampling subdivides the population into homogeneous subgroups and uses a probabilistic sampling method to sample within each stratum\n"),
         ...
         answer("We will obtain more precise estimates for a given total sample size if we sample in fewer places and measure more salmon than if we sample in more places and measure fewer salmon", correct = FALSE,message = "the design effect (DEFF) is the inflation of the standard error of the estimate when we use a probabilistic sampling method other than Simple Random Sampling. the DEFF is smaller if we sample in more places and take fewer measurements in each place than the opposite, because in the same place units tend to be more similar (so smaller variability) than for the overall population\n"),
         answer("Stratification can be performed according to a geographical area (e.g. squares or catchment boundaries), a river type or stream orders.  It all depends on your specific reasons for introducing a stratum into your sample. ", correct = TRUE, message = "All these, and possibly others, are possible strata\n"),
         random_answer_order = TRUE, allow_retry=TRUE)
```
